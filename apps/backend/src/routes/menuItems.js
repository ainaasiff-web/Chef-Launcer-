...
menuItemsRouter.post("/", authMiddleware, chefOnlyMiddleware, zValidator("json", createMenuItemSchema, (result, c) => {
    if (!result.success) {
...
menuItemsRouter.put("/:id", authMiddleware, chefOnlyMiddleware, zValidator("json", updateMenuItemSchema, (result, c) => {
    if (!result.success) {
        const issue = result.error.issues[0];
...
menuItemsRouter.delete("/:id", authMiddleware, chefOnlyMiddleware, async (c) => {
    try {
        const user = c.get("user");
        const id = c.req.param("id");
        const chefProfile = await safeDb.getChefProfileByUserId(user.id);
